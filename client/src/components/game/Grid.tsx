    ...
    <>
      {guess.split("").map((char, i) => (
        <motion.div
          ...
          animate={{ rotateX: 360 }}
          className={`
            aspect-square flex items-center justify-center font-bold text-base border-2
  ...
  return (
    <>
      {chars.map((char, i) => (
        <div
          key={i}
          className="aspect-square flex items-center justify-center font-bold text-base border-2 border-gray-300 w-10 h-10"
        >
          {char !== " " ? char : ""}
        </div>
      ))}
  ...
  return (
    <>
      {Array(solution.length).fill("").map((_, i) => (
        <div
          key={i}
          className="aspect-square border-2 border-gray-200 w-10 h-10"
        />
      ))}
    </>
  );
