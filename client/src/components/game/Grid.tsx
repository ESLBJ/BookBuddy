    ...
    <>
      {guess.split("").map((char, i) => (
        <motion.div
          ...
          animate={{ rotateX: 360 }}
          className={`
            h-12 flex items-center justify-center font-bold text-xl border-2
  ...
  return (
    <>
      {chars.map((char, i) => (
        <div
          key={i}
          className="h-12 flex items-center justify-center font-bold text-xl border-2 border-gray-300"
        >
          {char !== " " ? char : ""}
        </div>
      ))}
